PROGRAM Implicit_MPM_eartquake
      ...
      REAL(iwp),ALLOCATABLE:: equations(:,:),boundel(:),identidad(:,:),derextend(:,:),funCMPM(:),  &
                              beeCMPM(:,:),eldCMPM(:),beeextend(:,:),funextend2(:,:),funextend(:), &
                              jac_coord(:,:),gp(:,:),scalar_dee(:),dee_scal(:,:)
     ...
     !-----------------------input and initialisation--------------------------
      ...
      !READ(10,*)mbod(2)%name,mbod(2)%slopeopt,mbod(2)%w1,mbod(2)%h1,mbod(2)%s1, &
      !mbod(2)%nex,mbod(2)%ney,mbod(2)%dist_x,mbod(2)%dist_y,mbod(2)%np_types
      !mbod(2)%nx1=mbod(2)%nex
      !mbod(2)%ny1=mbod(2)%ney
      !mbod(2)%nprops=6 !For Linear Elastic == 3
      !ALLOCATE(mbod(2)%prop(mbod(2)%nprops,mbod(2)%np_types))
      !READ(10,*)mbod(2)%prop
      !mbod(2)%nx2=0;mbod(2)%ny2=0
      ...
      !PRINT*,'Use MPM:1'
      !PRINT*,'Use GIMP:2'
      !PRINT*,'Use GIMP to map & CMPM to recover:3'
      !READ(*,*)smethod
      smethod=3
...
END IF
  ...
  CALL GIMP_activenode(g_num,nip,g_coord,mbod(bod)%gm_coord,mbod(bod)%lp_mp,mbod(bod)%gimptol,neighb,mbod(bod)%a_ele,mbod(bod)%nf, &
                            mbod(bod)%GIMP_node_mp)
  CALL Elem_suport(mbod(bod)%c_ele,mbod(bod)%nf,g_num,cellsize,nx1,nip,nels,g_coord,mbod(bod)%gm_coord,mbod(bod)%lp_mp,  &
...
!---Compute mesh momentum and mass
 Fext_Mass_P: DO i=1,mbod(bod)%nmps
  ...
  IF(smethod==2.or.smethod==3)THEN 
   CALL GIMP_nodsup(i,g_num,nip,g_coord,mbod(bod)%gm_coord,mbod(bod)%lp_mp,mbod(bod)%mpoints,mbod(bod)%valuesg,mbod(bod)%gimptol,neighb, &
                       nny,mbod(bod)%a_ele,mbod(bod)%GIMP_nodes)
  ...
  ELSE    
     El_stiff:DO i=1,4 !This loop goes until 4 since 4 is the maximum number of elements affected by a material point
      ...
      Double_map:IF(smethod>1)THEN !-Double mapping technique
       CALL iGIMP_funder3(k,mbod(bod)%mpoints,mbod(bod)%lp_mp,nip,coord,cellsize,mbod(bod)%gm_coord,mbod(bod)%gimptol,nod_num,   &
                       g_coord,mbod(bod)%a_ele,mbod(bod)%c_ele,iel,der,fun)
...
MP_VelAccP: DO i=1,mbod(bod)%nmps
  ...
  IF(smethod==2.or.smethod==3)THEN 
   ...
   mbod(bod)%m_mvp(eldddylds)=mbod(bod)%m_mvp(eldddylds)+    &
                    MATMUL(MATMUL(funextend2,mbod(bod)%m_mass(i)*delta),mbod(bod)%m_velocity(:,i))
...
END IF
 ...
 END IF
    ...
    DEALLOCATE(mbod(bod)%d1x1,mbod(bod)%d2x1,mbod(bod)%diag,mbod(bod)%m_mvp,    &
      ...
      mbod(bod)%x1_change,mbod(bod)%f_earth,mbod(bod)%kinup_Ground_d2x1,mbod(bod)%cdamp, &
        mbod(bod)%mvkv)
