...
PROGRAM Tied_Free_Field_Basic
    ...
    Body_fixities: DO bod=1,size(mbod)
        ...
        DO iel=1,mbod(bod)%nels
            ...
            mbod(bod)%g(1:18:2)  = mbod(bod)%nf(1,num(:)) ! this one in x-dir
            mbod(bod)%g(2:18:2)  = mbod(bod)%nf(2,num(:)) ! this one in y-dir
        ...
        sub_step: DO sstep =1,substep
            ...
            Body_Solution: DO bod=1,size(mbod)
                ...
                Fext_domain_ff: DO k=1,mbod(bod)%nmps
                    ...
                    mbod(bod)%gravlo(mbod(bod)%g(1:18)) =                       &
                        mbod(bod)%gravlo(mbod(bod)%g(1:18)) +                   &
                        mbod(bod)%diag(mbod(bod)%g(1:18))*(-Gravf)  
            ...
            DO bod=1,1
                ...
                DO i=1,mbod(bod)%nn-1
                    ...
                    IF (                                                        &
                        (i<=mbod(bod)%ny1*2) .or.                               &
                        (i>(mbod(bod)%ny1*2+1)*(mbod(bod)%nex*2))               &
                    ) THEN
            ...
            Newton_Rhapson: DO WHILE(iters<=limit)        
                ...
                DO bod=1,size(mbod)
                    ...
                    Fint_load:DO i=1,mbod(bod)%nmps
                        ...
                        mbod(bod)%ddylds(mbod(bod)%g(1:18)) =                   &
                            mbod(bod)%ddylds(mbod(bod)%g(1:18)) +               &
                        ...
                        DO i=1,mbod(bod)%nn
                            IF(mbod(bod)%g_coord(2,i)<lowbound+0.01_iwp)THEN
                                mbod(bod)%loads(mbod(bod)%nf(1,i)) =            &
                                    ...
                                    mbod(2)%loads(mbod(2)%nf(1,mbod(1)%ny1*2+1))
                                m = m+1
                            END IF    
                        END DO
                    END IF
