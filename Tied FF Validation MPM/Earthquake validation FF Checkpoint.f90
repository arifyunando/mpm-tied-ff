PROGRAM IMPLICIT_TIED_FF_MPM
  ...
  REAL(iwp),ALLOCATABLE:: equations(:,:),boundel(:),identidad(:,:),            &
    derextend(:,:),funCMPM(:),beeCMPM(:,:),eldCMPM(:),beeextend(:,:),          &
    funextend2(:,:),funextend(:),jac_coord(:,:),gp(:,:),                       &
    scalar_dee(:),dee_scal(:,:)

  INTEGER,ALLOCATABLE::eldddylds(:),eqmat(:),eqmat1(:),eqmat2(:),              &
    neighb(:,:),nod_num(:,:)
  ...
  REAL(iwp):: tnph,tnps,alpha_2
  REAL(iwp):: zero=0.0_iwp,one=1.0_iwp,two=2.0_iwp,d3=3.0_iwp,d180=180.0_iwp
  ...
  TYPE::mpm_body
    CHARACTER(10):: name
    INTEGER:: A,Bi,emps,newnodes,nyp,newel,nmps,nn
    INTEGER:: slopeopt,slopeel,slope1,tep
              
    REAL(iwp):: w1,s1,h1
    REAL(iwp):: Young,Poiss,frictfact
    INTEGER:: nex,ney,nels,locx,locy,ale,neq,np_types,nprops,                  &
              nx1,nx2,ny1,ny2,dist_x,dist_y
    INTEGER,ALLOCATABLE:: b(:),etype(:),g(:),g_g(:,:),g_num(:,:),nf(:,:),      &
      num(:),MPPE(:),neighb(:,:),nodecont(:),newels(:),valuesg(:),             &
      boundnod(:),kdiag(:),kconst(:),nodecont_f(:),tied_nn(:,:),base_nn(:)
    ...
    REAL(iwp),ALLOCATABLE:: a_field(:,:),ddylds(:),diag(:),d1x1(:),d2x1(:),    &
      ...
      v_field(:,:),vcm(:),vel_change(:),x1(:),x1_orig(:),x1_change(:),x1_ini(:),&
      f_earth(:),kinup_Ground_d2x1(:),cdamp(:),mvkv(:)
  ...
  !-----------------------------mpm integers----------------------------------
  ! These are only used for mesh generation. 
  ! Each body will be generated separately, so need to be in body structure.
  ...
  bod=1          
  READ(10,*) mbod(bod)%name,                                      & ! Block  
    mbod(bod)%slopeopt, mbod(bod)%w1, mbod(bod)%h1, mbod(bod)%s1, & ! 2  2.0 10.0  0.0  
    mbod(bod)%nex, mbod(bod)%ney,                                 & ! 2 10
    mbod(bod)%dist_x, mbod(bod)%dist_y,                           & ! 2 1
    mbod(bod)%np_types                                              ! 1
  ...
  READ(10,*) mbod(bod)%prop ! 2.65  5.0e3  0.30  1000.0  3.0  20.0  

  mbod(bod)%nx2=0; mbod(bod)%ny2=0
      
  !***************************** PROGRAM SETTINGS *****************************!
  !                                                                            !
  ! Select "smethod" which indicats the use of MPM(smethod=1), GIMP(smethod=2), 
  ! or CMPM (smethod=3). It is recomended to use or smethod=1 or smethod=3
  smethod=3 ! Use GIMP to map & CMPM to recover

  ! Use Gravity in bod 1:Yes 2:No
  gravbod1=1

  ! The plastic loop jis to consider plasticity. 
  ! ploop = 1 consider plasticity, any other value does not
  ploop=0
  ...
  mbod(bod)%newel=mbod(bod)%ney      ! Number of elements in the x direction of the slope section
  !dist_x=dist_x+mbod(bod)%newel+nx1 ! Separation between body 1 and 2 (in elements, not in distance)
  !dist_y=ny1-mbod(2)%ney            ! Separation between upper boundary and upper element of the wall
  mbod(bod)%slopeel=0
  ...
  ALLOCATE(g(ndof),weights(nip),points(nip,ndim),num(nod),coord(nod,ndim),      &
  ...
  ALLOCATE(m_num(nip),vcc(ndof),ini_volume(nels),m_coord(nip,ndim),gc(ndim))
  ...
  DO bod=1,size(mbod)
    ...
    !IF(mbod(bod+1)%dist_x+mbod(bod+1)%nx1>mbod(bod)%dist_x+mbod(bod)%nx1)dist_x=mbod(bod+1)%dist_x+mbod(bod+1)%nx1
    !IF(mbod(bod+1)%dist_y+mbod(bod+1)%ny1>mbod(bod)%dist_y+mbod(bod)%ny1)dist_y=mbod(bod+1)%dist_y+mbod(bod+1)%ny1   
  END DO  
  ...
  Body_fixities: DO bod=1,size(mbod)
    ...
    !---------------------------------------------------------------------------
      ...
      !rowy=1
      !n=1
      !!- Findin left degree in function of mass
      !DO i=1,nn
      !    m=((colx-1)*(nny))+rowy
      !    IF(mbod(bod)%nf(1,m)>0)THEN
      !        mbod(bod)%tied_nn(n,1)=m
      !        colx=colx+1
      !        m=((colx-1)*(nny))+rowy
      !        mbod(bod)%tied_nn(n,2)=m
      !        colx=colx+1
      !        m=((colx-1)*(nny))+rowy
      !        mbod(bod)%tied_nn(n,3)=m
      !        rowy=rowy+1
      !        n=n+1
      !        colx=0
      !    END IF   
      ...
      !    IF(n>nny)EXIT
      !END DO  
      !!- Finding right degree in function of mass
      !colx=nnx
      !rowy=1
      !n=1
      !DO i=1,nn
      !    m=((colx-1)*(nny))+rowy    
      !    IF(mbod(bod)%nf(1,m)>0)THEN
      !        mbod(bod)%tied_nn(n,6)=m
      !        colx=colx-1
      !        m=((colx-1)*(nny))+rowy
      !        mbod(bod)%tied_nn(n,5)=m
      !        colx=colx-1
      !        m=((colx-1)*(nny))+rowy
      !        mbod(bod)%tied_nn(n,4)=m
      !        rowy=rowy+1
      !        colx=nnx+1
      !        n=n+1
      !    END IF   
  ...
  READ(10,*)dtim, k0, aiters, tol, limit, nsrf ! 0.001  0.50  1  1.0e-8 50 0.1
  ...
  !     mbod(bod)%m_stress(1,i)=mbod(bod)%prop(1,1)*Gravf*(mbod(bod)%gm_coord(2,i)-HSURF)*k0!  For clays Ko=0.70      ((prop(7,1))/(1-prop(7,1)))
  !     mbod(bod)%m_stress(2,i)=mbod(bod)%prop(1,1)*Gravf*(mbod(bod)%gm_coord(2,i)-HSURF)
  ...
  CALL point_viz2(0,1,argv,mbod(bod)%gm_coord,mbod(bod)%m_stress,   &
    mbod(bod)%m_stress_change,mbod(bod)%epsinvacum,mbod(bod)%a_ins, &
    mbod(bod)%Devstress,mbod(bod)%mean_stress,mbod(bod)%mpyield,    &
    mbod(bod)%mpcp,mbod(bod)%m_velocity,mbod(bod)%m_acc,mbod(bod)%nmps,nlen)
  CALL paraview2(cont2,1,argv,g_coord,g_num,mbod(bod)%nf,nels,nod,nn,nlen, &
    ...
    mbod(bod)%gravlo,mbod(bod)%loads,mbod(bod)%normal,mbod(bod)%fcont,     &
    mbod(bod)%kv,mbod(bod)%mv,mbod(bod)%kdiag,mbod(bod)%vcm,mbod(bod)%f_fint)        
  ...
  !!============================================================================
  !! --------------------- BEGINNING OF THE TIME STEPS -------------------------
  !!============================================================================

  step=0
  time_steps: DO w=1,100000
    PRINT '("Steps :" (I5) "/" (I4))', step, 100000
    step=step+1 
        ...
        Fext_Mass_P: DO i=1,mbod(bod)%nmps
          IF(mbod(bod)%nmps>1)THEN 
            ...
            IF(smethod==2.or.smethod==3)THEN 
              CALL GIMP_nodsup(i,g_num,nip,g_coord,mbod(bod)%gm_coord, &
                mbod(bod)%lp_mp,mbod(bod)%mpoints,mbod(bod)%valuesg,   &
                mbod(bod)%gimptol,neighb,nny,mbod(bod)%a_ele,mbod(bod)%GIMP_nodes)
              ...
              !IF((bod==1.and.gravbod1==2).or.(bod==2.and.gravbod2==2))THEN
              !  mbod(bod)%gravlo(eldddylds)=mbod(bod)%gravlo(eldddylds)+  &
              !                  MATMUL(funextend2,mbod(bod)%m_mass(i)*MATMUL(delta,g_matrix))  
              !ELSE
              !   mbod(bod)%gravlo(eldddylds)=mbod(bod)%gravlo(eldddylds)+  &
              !                  MATMUL(funextend2,mbod(bod)%m_mass(i)*MATMUL(delta,mbod(bod)%g_matrix))  
              !END IF
            ...
            ELSE
              ...
              !DO m=1,nod
              ! DO n=1,nodof
              !  nt((m-1)*nodof+n,n)=fun(m)
              ! END DO
              !END DO
              !IF(bod==1.and.gravbod1==2)THEN
              !  mbod(bod)%gravlo(g)=mbod(bod)%gravlo(g)+MATMUL(nt,mbod(bod)%m_mass(i)*MATMUL(delta,g_matrix))  
              !ELSE
              !  mbod(bod)%gravlo(g)=mbod(bod)%gravlo(g)+MATMUL(nt,mbod(bod)%m_mass(i)*MATMUL(delta,mbod(bod)%g_matrix))
              !END IF
              !mbod(bod)%m_mvp(0)=zero;mbod(bod)%diag(0)=zero;mbod(bod)%gravlo(0)=zero;mbod(bod)%m_mva(0)=zero
            END IF
          END IF
    ...
    !-- Compute domain stiffnes matrices and domain mass matrices
        ...
        KM_MV:DO k=1,mbod(bod)%nmps
          ...
          IF(mbod(bod)%kconst(1)==1)THEN
            ...
            K_const:DO iel=1,nels 
              ...
              num=g_num(:,iel)
              !-The next conditin is to find if the element is under the influence 
              ! of any material point in the domain. If is influenced, then the FEM 
              ! stiffnes is computed for that element
            ...
            El_stiff:DO i=1,4 !This loop goes until 4 since 4 is the maximum number of elements affected by a material point
              ...
              Full_el:IF(mbod(bod)%elemmpoints(k,i)>0)THEN !--If is true, an element is affected by a material point (material point can be outside the element)
                ...
                Double_map:IF(smethod>1)THEN !-Double mapping technique
                  CALL iGIMP_funder3(k,mbod(bod)%mpoints,mbod(bod)%lp_mp,nip,coord,cellsize,mbod(bod)%gm_coord,mbod(bod)%gimptol,nod_num,g_coord,mbod(bod)%a_ele,mbod(bod)%c_ele,iel,der,fun)
                  ...
                  DO s=1,nip
                    ...
                    IF(mbod(bod)%c_ele(iel)<1)waverage=1.5_iwp!4.0_iwp/(REAL(mbod(bod)%c_ele(iel)))
                    km_gauss=km_gauss+MATMUL(MATMUL(TRANSPOSE(bee_gauss),dee_scal),bee_gauss)*det*weights(i)!*waverage!*(4.0*mbod(bod)%lp_mp(1,k)*mbod(bod)%lp_mp(2,k))!*mbod(bod)%m_volume(k)*waverage
                  END DO
          ...
          DO j=1,values
            ...
            END DO
          END DO
          !mm_gimp=mm_gimp*(4.0*mbod(bod)%lp_mp(1,k)*mbod(bod)%lp_mp(2,k))/(cellsize/2.0)**2
    ...
    !Equilibrium_iter: DO bod2=1,size(mbod)
        ...
        IF(mbod(bod)%nmps>1)THEN
          MatPoints: DO i=1,mbod(bod)%nmps
            ...
            ELSE IF(smethod==2)THEN
              values=mbod(bod)%valuesg(i)
              ALLOCATE(derextend(nodof,values),funextend(values),beeextend(nst,values*nodof),eldCMPM(values*nodof),jac_coord(values,nodof))
            ...
            Yieldf:IF(fnew>zero.and.ploop==1)THEN
              ...
              iterations1:DO WHILE(Tret<one)
                ...
                ELSE                    
                  ...
                  IF(ABS(f)>ftol)THEN
                    CALL yieldcorrection(stress,epsinv,cpeak,0.0_iwp,cpeak,Maxplastic,mpcr,0.0_iwp,SModulus,mbod(bod)%dee,ft)
                  END IF
    ...
    !UPGRADE_LP: DO bod=1,size(mbod)
    !      IF(mbod(bod)%nmps>1)THEN 
    !   DO i=1,mbod(bod)%nmps 
    !    mbod(bod)%lp_mp(1,i)=mbod(bod)%lp_mp(1,i)+(mbod(bod)%lp_mp(1,i)*mbod(bod)%eps_m(1,i))
    !    mbod(bod)%lp_mp(2,i)=mbod(bod)%lp_mp(2,i)+(mbod(bod)%lp_mp(2,i)*mbod(bod)%eps_m(2,i))
    !    mbod(bod)%deeinv=mbod(bod)%dee
    !    CALL invert(mbod(bod)%deeinv)
    !    mbod(bod)%eps_acum(:,i)=MATMUL(mbod(bod)%m_stress(:,i),mbod(bod)%deeinv)
    !   END DO
    !   END IF
    !END DO UPGRADE_LP
    ...
    REVERSE_MAPPING: DO bod=1,size(mbod)
      ...
      IF(mbod(bod)%nmps>1)THEN 
        DO i=1,mbod(bod)%nmps
          ...
          !  acc=mbod(bod)%d2x1(g)
          !  vcc=mbod(bod)%d1x1(g)
          !  !IF(ABS(vcc(1)).NE.0.0)PAUSE
          !  num=g_num(:,iel)
          !  coord=TRANSPOSE(g_coord(:,num))
          !  CALL shape_fun(fun,mbod(bod)%mpoints,i)
          !  !** Update material point velocity
          !    
          !  mbod(bod)%accp(1,i)=DOT_PRODUCT(acc(1:ndof:2),fun)
          !  mbod(bod)%accp(2,i)=DOT_PRODUCT(acc(2:ndof:2),fun)
          !
          !  mbod(bod)%m_acc(:,i)=mbod(bod)%accp(:,i)
          !  mbod(bod)%m_velocity(:,i)=mbod(bod)%m_velocity(:,i)+mbod(bod)%accp(:,i)*dtim
          !    
          !  !** Convect material points
          !  mbod(bod)%vccp(1,i)=DOT_PRODUCT(vcc(1:ndof:2),fun)
          !  mbod(bod)%vccp(2,i)=DOT_PRODUCT(vcc(2:ndof:2),fun)
          !  mbod(bod)%ins(:,i)=mbod(bod)%vccp(:,i)*dtim
          !  mbod(bod)%a_ins(:,i)=mbod(bod)%a_ins(:,i) + mbod(bod)%ins(:,i)
          !  mbod(bod)%gm_coord(:,i)=mbod(bod)%gm_coord(:,i)+mbod(bod)%ins(:,i)
          !     
          !ELSE

          ALLOCATE(derextend(nodof,values),beeextend(nst,values*nodof),funextend2(values*2,2),eldddylds(values*2),funextend(values),jac_coord(values,nodof),eldCMPM(values*2))
      ...
      IF(mbod(bod)%nmps>1)THEN 
        IF(cont2/printval*printval==cont2)THEN
          ...
          CALL paraview2(cont2,1,argv,g_coord,g_num,mbod(bod)%nf,nels,nod,nn,nlen,            &
                ...
                mbod(bod)%gravlo,mbod(bod)%x1,mbod(bod)%normal,mbod(bod)%fcont,mbod(bod)%kv,mbod(bod)%mv,&
                mbod(bod)%kdiag,(-1.0)*mbod(bod)%vcm,mbod(bod)%f_fint)        
          
          CALL point_viz2(cont2,1,argv,mbod(bod)%gm_coord,mbod(bod)%m_stress,mbod(bod)%m_stress_change,  &
      ...
      ChangeEl: DO bod=1,size(mbod)
        IF(mbod(bod)%nmps>1)THEN  
          ...
          ELSE
            ...
            CALL GIMP_activenode(g_num,nip,g_coord,mbod(bod)%gm_coord,mbod(bod)%lp_mp,mbod(bod)%gimptol,neighb,mbod(bod)%a_ele,mbod(bod)%nf,mbod(bod)%GIMP_node_mp)
            CALL Elem_suport(mbod(bod)%c_ele,mbod(bod)%nf,g_num,cellsize,nx1,nip,nels,g_coord,mbod(bod)%gm_coord,mbod(bod)%lp_mp,mbod(bod)%gimptol,smethod,mbod(bod)%elemmpoints)
          END IF
          ...
          DEALLOCATE(mbod(bod)%d1x1,mbod(bod)%d2x1,mbod(bod)%diag,mbod(bod)%m_mvp,    &
            ...
            mbod(bod)%x1_orig,mbod(bod)%f_fint,mbod(bod)%vel_change,mbod(bod)%x1_ini, &
          ...
          ALLOCATE(mbod(bod)%d1x1(0:mbod(bod)%neq),mbod(bod)%d2x1(0:mbod(bod)%neq),   &
            ...
            mbod(bod)%kinup_d1x1(0:mbod(bod)%neq),mbod(bod)%x1_orig(0:mbod(bod)%neq), &
            mbod(bod)%f_fint(0:mbod(bod)%neq),mbod(bod)%vel_change(0:mbod(bod)%neq),  & 
  ...
  END DO time_steps
  STOP
END PROGRAM IMPLICIT_TIED_FF_MPM
