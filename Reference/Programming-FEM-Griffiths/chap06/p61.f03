...
!-----------------------plastic iteration loop----------------------------
   its: DO
     ...
     DO i=1,loaded_nodes 
       loads(nf(:,node(i)))=val(i,:)*qinc(iy)   	                            ! input boundary conditions
     END DO
     loads=loads+bdylds 
...
!-----------------------check whether yield is violated-------------------
         f=dsbar-SQRT(d3)*prop(1,etype(iel))                                    ! von mises yield function [p238]
         ...
         ELSE
           IF(f>=zero)THEN
             ...
             flow=f*(m1*dq1+m2*dq2+m3*dq3)                                      ! dQ/dsigma
             erate=MATMUL(flow,stress)                                          ! viscoplastic strain rate (eps_dot vp) (eq. 6.19)
             evp=erate*dt                                                       ! Viscoplastic strain rate increment (eq. 6.20)
             evpt(:,i,iel)=evpt(:,i,iel)+evp                                    ! running total of visplastic strains (eq. 6.21)
             devp=MATMUL(dee,evp)                                               ! product of Elastic Stiffness * viscoplastic strains (eq. 6.26)
           END IF
         END IF
         IF(f>=zero.OR.(converged.OR.iters==limit))THEN
           eload=MATMUL(devp,bee)                                               ! Integration point contribution to bload (eq. 6.26)
           bload=bload+eload*det*weights(i)                                     ! correcting force based on viscoeplastic strain (eq. 6.26)
         END IF
!-----------------------update the Gauss Point stresses-------------------
         IF(converged.OR.iters==limit)tensor(:,i,iel)=stress
       END DO gauss_pts_2
!-----------------------compute the total bodyloads vector----------------
       bdylds(g)=bdylds(g)+bload                                                ! eq 6.26
       bdylds(0)=zero
     END DO elements_3 
