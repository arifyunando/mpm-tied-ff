...
!$$$$$$            wt=1.0_iwp
     ...
     SUBROUTINE sample3(element,s,wt,nipslo)
     !
     ! This subroutine returns the local coordinates and weighting coefficients
     ! of the integrating points.
     !
     ...
     END SUBROUTINE checon
       ...
       SUBROUTINE checon_1(DuC,DuP,converged,tol)
     !
     ! This subroutine sets converged to .FALSE. if relative change in loads
     ! and oldlds is greater than tol and updates oldlds.
     !
    ...
    !DO i=1,SIZE(DuP)-1
      ...
      !CONVERGED=(upvalB1/upvalB2<=tol)
      !IF(ABS(upvalB1)<1e-7.and.ABS(upvalB2)<1e-7)THEN
      !    CONVERGED=.true.
      !ELSE    
...
END SUBROUTINE vmflow
   ...
   END SUBROUTINE plasticmod
    ...
    SUBROUTINE mocouq_Sloan(coh,psi,dsbar,theta,dq1,dq2,dq3,TLoad,a_slo)
     !
     ! This subroutine forms the derivatives of a Mohr-Coulomb potential
     ! function with respect to the three invariants (psi in degrees).
     !
      IMPLICIT NONE
      INTEGER,PARAMETER::iwp=SELECTED_REAL_KIND(15)
      REAL(iwp),INTENT(IN)::psi,dsbar,theta,TLoad,a_slo,coh
      REAL(iwp),INTENT(OUT)::dq1,dq2,dq3
      REAL(iwp)::psir,snth,snps,sq3,c1,csth,cs3th,tn3th,tnth,zero=0.0_iwp,     &
        pt49=0.49_iwp,pt5=0.5_iwp,one=1.0_iwp,d3=3.0_iwp,d4=4.0_iwp,           &
        d180=180.0_iwp,sign_t,DTDT,Bval,CMC1,CMC2,CMC3,KT,alpha,i3=3.0_iwp,    &
        two=2.0_iwp,LOADTRad,p3,Aval,pi,afact,cotps
      ...
      p3=1.0_iwp/3.0_iwp
      cotps=one/TAN(psir)
      afact=a_slo*coh*cotps
     
      IF(theta<=LOADTRad)THEN
        DTDT=(-snth/SQRT(d3)-snps*csth/d3)
        KT=(csth/SQRT(d3)-snth*snps/d3)
      ELSE
        IF(theta>=0.0)sign_t=1.0
        IF(theta<0.0)sign_t=-1.0  
   
        Bval=(one/(i3*COS(i3*LOADTRad)))*(sign_t*SIN(LOADTRad)+ &
              (one/SQRT(i3))*snps*COS(LOADTRad))
        Aval=p3*COS(LOADTRad)*(i3+TAN(LOADTRad)*TAN(i3*LOADTRad)+    &
             one/SQRT(i3)*sign_t*(TAN(i3*LOADTRad)-i3*TAN(LOADTRad))*snps)
        KT=  (Aval- Bval*SIN(i3*theta))*(1.0_iwp/SQRT(i3))  
        DTDT=(-i3*Bval*COS(i3*theta))*(1.0_iwp/SQRT(i3)) 
      END IF
       alpha=dsbar*KT/SQRT(dsbar**2*KT**2+afact**2*snps**2)
	   CMC2=3.0/dsbar*pt5*(KT-TAN(i3*theta)*DTDT)*alpha
       CMC3=-9.0/(two*COS(i3*theta)*(dsbar)**2)*DTDT*alpha
       dq1=CMC1
       dq2=CMC2
       dq3=CMC3
