...
!$$$$$$        CASE(4)
!$$$$$$         ! Linear element .....
!$$$$$$         aa=coord(2,:)+n*(coord(1,:)-coord(2,:))/npart
!$$$$$$         bb=coord(2,:)+(n-1)*(coord(1,:)-coord(2,:))/npart
!$$$$$$         cc=coord(3,:)+n*(coord(4,:)-coord(3,:))/npart
!$$$$$$         dd=coord(3,:)+(n-1)*(coord(4,:)-coord(3,:))/npart
!$$$$$$         s_coord(1,:)=aa+(m-1)*(cc-aa)/mpart
!$$$$$$         s_coord(2,:)=bb+(m-1)*(dd-bb)/mpart
!$$$$$$         s_coord(3,:)=bb+m*(dd-bb)/mpart
!$$$$$$         s_coord(4,:)=aa+m*(cc-aa)/mpart
!$$$$$$       CASE DEFAULT
!$$$$$$           WRITE(*,*)"wrong number of nodes in s_coord"
!$$$$$$       END SELECT
 ...
 SUBROUTINE point_viz2(input,realisation,argv,gm_coord,m_stress,m_stress_ini,evpt,a_ins,Devstress,   &
   ...
   !---- SUBROUTINE used to save visualise outputs to Paraview format ---------------------
    ...
    ! estress: elastic stress?!
     ...
     !open(ss,FILE=argv(1:nlen)//"_"//trim(adjustl(cnumber1))//"_"//trim(adjustl(cnumber))//'.inp')
     ss=15
     OPEN(ss,FILE="Paraview2/"//argv(1:nlen)//"_"//trim(adjustl(cnumber1))//"_"//trim(adjustl(cnumber))//'.inp') ! Creates a file with title argv_cnumber1_cnumber
     write(ss,*) '#'
     write(ss,*) '# Simple AVS UCD File'
     write(ss,*) '#'
     write(ss,'(2i7.1,a)') nmps, nmps, ' 3 0 0' ! Number of nodes, number of cells, number of data items per node, number of data items per cell, model number (always 0)
...
!** SUBROUTINE read_mps used to read mp locations from a file
    ...
    SUBROUTINE shapefunct_c(points,i,CONSTANTS,SFUNCT,bound)
     !
     !   This subroutine produces derivatives of shape functions withe respect
     !   to local coordinates for 16 and 9 nodes.
     !
      ...
      eta=points(i,2)
      IF(bound==4)THEN
        xi=points(i,1)-2.0_iwp
        eta=points(i,2)
      ELSE IF(bound==6)THEN
        xi=points(i,1)+2.0_iwp
        eta=points(i,2)
      ELSE IF(bound==7)THEN
        xi=points(i,1)-2.0_iwp
        eta=points(i,2)-2.0_iwp
      ELSE IF(bound==8)THEN
        xi=points(i,1)
        eta=points(i,2)-2.0_iwp
      ELSE IF(bound==9)THEN
        xi=points(i,1)+2.0_iwp
      ...
      END IF
     RETURN
     END SUBROUTINE shapefunct_c

     SUBROUTINE derivatives_c(points,i,CONSTANTS,SIXTEEND,bound)
     !
     !   This subroutine produces derivatives of shape functions withe respect
     !   to local coordinates for 16 and 9 nodes.
     !
     ...
     SUBROUTINE eldform_c(eld,loads,iel,neighb,values,g_g,bound,m)
   ...
   location=5
     ...
     !ELSE IF(bound==6.and.m==2)THEN
      ...
      !ELSE IF(bound==9)THEN
        !bound 9= right lowest corner
      !ELSE IF(location==5)THEN
         !bound 5= center element
         b=g_g(3:6,neighb(ele,1))
         eld(n:n+3)=loads(b+1)
         n=n+2
         b=g_g(3:6,neighb(ele,2))
         eld(n:n+3)=loads(b+1)
         n=n+2
         b=g_g(3:6,neighb(ele,3))
         eld(n:n+3)=loads(b+1)
         n=n+4
         b=g_g(3:6,neighb(ele,8))
         eld(n:n+3)=loads(b+1)
         n=n+2
         b=g_g(3:6,ele)
         eld(n:n+3)=loads(b+1)
         n=n+2
         b=g_g(3:6,neighb(ele,4))
         eld(n:n+3)=loads(b+1)
         n=n+4
         b=g_g(3:6,neighb(ele,7))
         eld(n:n+3)=loads(b+1)
         n=n+2
         b=g_g(3:6,neighb(ele,6))
         eld(n:n+3)=loads(b+1)
         n=n+2
         b=g_g(3:6,neighb(ele,5))
         eld(n:n+3)=loads(b+1)
         n=n+4
         b(1)=g_g(1,neighb(ele,7))
         b(2)=g_g(2,neighb(ele,7))
         b(3)=g_g(7,neighb(ele,7))
         b(4)=g_g(8,neighb(ele,7))
         eld(n:n+3)=loads(b+1)
         n=n+2
         b(1)=g_g(1,neighb(ele,6))
         b(2)=g_g(2,neighb(ele,6))
         b(3)=g_g(7,neighb(ele,6))
         b(4)=g_g(8,neighb(ele,6))
         eld(n:n+3)=loads(b+1)
         n=n+2
         b(1)=g_g(1,neighb(ele,5))
         b(2)=g_g(2,neighb(ele,5))
         b(3)=g_g(7,neighb(ele,5))
         b(4)=g_g(8,neighb(ele,5))
         eld(n:n+3)=loads(b+1)
      !END IF

 RETURN
 END SUBROUTINE eldform_c
