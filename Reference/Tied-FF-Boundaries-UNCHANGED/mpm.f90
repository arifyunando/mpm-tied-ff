...
!         END DO
  ...
  SUBROUTINE paraview2(input,realisation,argv,nff,nelsy,g_coord,g_num,nf,nels,nod,nn,nlen,  &
      ...
      ELSE IF(bod==2)THEN
          OPEN(ss,FILE="Paraview_2DM_1/"//argv(1:nlen)//"_"//trim(adjustl(cnumber1))//"_"//trim(adjustl(cnumber))//'.vtk')
      ELSE
          OPEN(ss,FILE="Paraview_2DM_2/"//argv(1:nlen)//"_"//trim(adjustl(cnumber1))//"_"//trim(adjustl(cnumber))//'.vtk')
      END IF
 ...
 SUBROUTINE point_viz2(input,realisation,argv,gm_coord,m_stress,m_eps_acum,evpt,a_ins,Devstress,   &
   ...
   !---- SUBROUTINE used to save visualise outputs to Paraview format ---------------------
    ...
    ! estress: elastic stress?!
     ...
     ELSE IF(bod==2)THEN
      OPEN(ss,FILE="Paraview_2DP_1/"//argv(1:nlen)//"_"//trim(adjustl(cnumber1))//"_"//trim(adjustl(cnumber))//'.inp') ! Creates a file with title argv_cnumber1_cnumber   
     ELSE
      OPEN(ss,FILE="Paraview_2DP_2/"//argv(1:nlen)//"_"//trim(adjustl(cnumber1))//"_"//trim(adjustl(cnumber))//'.inp') ! Creates a file with title argv_cnumber1_cnumber   
     END IF
